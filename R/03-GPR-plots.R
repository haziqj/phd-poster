...
dev_FBMkern <- function(theta, y = y) {
  alpha <- mean(y)
  lambda <- exp(theta[1])
  psi <- exp(theta[2])
  # theta[3] <- 0
...
dev_Cankern <- function(theta, y = y) {
  alpha <- mean(y)
  lambda <- exp(theta[1])
  psi <- exp(theta[2])
  n <- length(y)
...
plot1 <- function(kernel = "SE", no.of.draws = 100) {
  # Fit an I-prior model -------------------------------------------------------
  if (kernel == "SE") {
    mod <- optim(c(1, 1, 1), dev_SEkern, method = "L-BFGS", y = y)
    n <- length(y)
    H <- fnH4(x, l = exp(mod$par[3]))
    alpha <- mean(y)
    lambda <- exp(mod$par[1])
    psi <- exp(mod$par[2])
    Vy <- lambda * H + diag(1 / psi, n)
    w.hat <- solve(Vy, y - alpha)
    ...
    Vf.pri <- lambda * fnH4(x.true, l = exp(mod$par[3]))

    # Posterior variance for f
    Vf.pos <- lambda * fnH4(x.true, l = exp(mod$par[3])) -
      lambda ^ 2 * H.star %*% solve(Vy, t(H.star))
    Vf.pos <- (Vf.pos + t(Vf.pos)) / 2
  } else if (kernel == "FBM") {
    mod <- optim(c(1, 1, 0), dev_FBMkern, method = "L-BFGS", y = y)
    n <- length(y)
    ...
    # Prior variance for f
    Vf.pri <- lambda * fnH3(x.true, gamma = expit(mod$par[3]))
    class(Vf.pri) <- NULL

    # Posterior variance for f
    Vf.pos <- lambda * fnH3(x.true, gamma = expit(mod$par[3])) -
      (lambda ^ 2) * H.star %*% solve(Vy, t(H.star))
    Vf.pos <- (Vf.pos + t(Vf.pos)) / 2
    class(Vf.pos) <- NULL
  } else if (kernel == "Canonical") {
    mod <- optim(c(1, 1), dev_Cankern, method = "L-BFGS", y = y)
    n <- length(y)
    ...
    # Prior variance for f
    Vf.pri <- lambda * fnH2(x.true); class(Vf.pri) <- NULL

    # Posterior variance for f
    Vf.pos <- lambda * fnH2(x.true) -
      (lambda ^ 2) * H.star %*% solve(Vy, t(H.star))
    Vf.pos <- (Vf.pos + t(Vf.pos)) / 2
    class(Vf.pos) <- NULL
  }
  ...
  VarY.hat <- (lambda / psi) * solve(Vy, H)
