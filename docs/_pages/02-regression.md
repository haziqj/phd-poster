...
gallery:
  - url: /images/post_reg_prior.png
    image_path: /images/post_reg_prior_th.png
    #title: "Random draws of the I-prior regression function from the fBm RKHS."
  - url: /images/post_reg_posterior.png
    image_path: /images/post_reg_posterior_th.png
    #title: "Random draws of the posterior regression function from the fBm RKHS under an I-prior."
  - url: /images/post_reg_cred.png
    image_path: /images/post_reg_cred_th.png
    #title: "Credibility interval for predictions of data points."
  - url: /images/post_reg_ppc.png
    image_path: /images/post_reg_ppc_th.png
...
One of the advantages of I-prior regression is that it provides a unifying methodology for a variety of statistical models. This is done by selecting (an) appropriate RKHS for the regression problem at hand. Here we illustrate several models based on the regression model
...
and an I-prior on $$f \in \mathcal F$$.

### Linear Regression

<!-- <figure style="width: 500px"> -->
<figure>
  <a href="/images/reg_lin.png"><img src="/images/reg_lin_th.png"></a>
...
where each $$f_j \in \mathcal F_j$$, and take each $$\mathcal F_j$$ to be the canonical linear RKHS, each with kernel $$\lambda_j h_j$$. The kernel
...
<figure>
  <a href="/images/reg_fbm.png"><img src="/images/reg_fbm_th.png"></a>
...
<figure class="half">
  <a href="/images/reg_multi_int.png"><img src="/images/reg_multi_int_th.png"></a>
  <a href="/images/reg_multi_slope.png"><img src="/images/reg_multi_slope_th.png"></a>
  <figcaption>Fitted regression line for a varying intercept model (left) and a varying slope model (right) under an I-prior.</figcaption>
</figure>

Suppose we had observations $$\{ (y_{ij}, x_{ij}) \}$$ for each unit or individual $$i$$ in group $$j \in \{ 1,\dots,m \}$$. We can model this using the function
...
which would then be similar to the multilevel model, except that we don't necessarily have to assume a nominal effect of time. We can choose either a linear or smooth effect of time by choosing the canonical or fBm RKHS respectively for $$f_2$$.
...
{% include gallery caption="Working in a Bayesian framework allows us to draw sample functions from the prior and posterior, obtain credibility intervals from the posterior distribution, and perform posterior predictive checks." %}
...
$$
\sigma^2 (x) = \mathbf{h}_\lambda^\top(x) \big(H_\lambda \Psi H_\lambda + \Psi^{-1}\big)^{-1} \mathbf{h}_\lambda(x).
$$

The posterior mean $$\mu(x)$$ can then be taken as a point estimate for $$f(x)$$, the evaluation of the function $$f$$ at a point $$x$$. As this has a distribution, we can also obtain credible intervals for the estimate. The equations above can also be used to obtain covariances between two point estimates $$f(x)$$ and $$f(x')$$.

## Computational Hurdles

Computational complexity is dominated by a $$n \times n$$ matrix inversion which is $$O(n^3)$$. In the case of Newton-based approaches, this needs to be evaluated at each Newton step. In the case of the EM algorithm, every update cycle involves such an inversion. For stochastic MCMC sampling methods, the inversion occurs in each sampling step.

Suppose that $$H_\lambda \Psi H_\lambda = QQ^\top$$, where $$Q$$ is a $$n \times q$$ matrix, is a valid low-rank decomposition. Then
